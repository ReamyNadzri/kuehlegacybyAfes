...
def get_video_duration(video_path):
    """Return video duration in seconds (float) using ffprobe or ffmpeg fallback."""
    try:
        cmd = [
            "ffprobe", "-v", "error",
    ...
    # Fallback using ffmpeg -i info output parsing
    ffmpeg_path = shutil.which("ffmpeg") or r"C:\Program Files\Softdeluxe\Free Download Manager\ffmpeg.exe"
    ...
    return 8.0 # hardcoded fallback


def get_frame_dimensions(frame_path):
    """Return (width, height) of a JPEG frame using ffprobe or ffmpeg fallback."""
    try:
        cmd = [
            "ffprobe", "-v", "error",
            "-select_streams", "v:0",
    ...
    # Fallback using ffmpeg -i info output parsing
    ffmpeg_path = shutil.which("ffmpeg") or r"C:\Program Files\Softdeluxe\Free Download Manager\ffmpeg.exe"
    cmd = [ffmpeg_path, "-i", frame_path]
    result = subprocess.run(cmd, capture_output=True, text=True)
    match = re.search(r"Video:.*?\s(\d+)x(\d+)", result.stderr)
    if match:
        return int(match.group(1)), int(match.group(2))
    return 360, 203 # hardcoded fallback
...
def extract_frames(
    ...
    if not os.path.exists(ffmpeg_path):
        raise EnvironmentError("ffmpeg not found on PATH or Free Download Manager directory.")
